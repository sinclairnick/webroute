...
Fundamentally, `webroute` consists of a single function, `route`, which is a poweful, simple and portable abstraction for atomically defining all aspects of an API endpoint.

<Callout>
### `webroute` is not a framework
 Instead, `webroute` happily enhances existing frameworks, or can be used without one.
</Callout>
...
Declarative approach enables API contract and schema definitions for free
...
Each route is complete and standalone – `webroute`s are router agnostic. Upstream operations like middleware are always traceable and baked in.
...
`webroute`s are inherently immutable, never mutating request or response objects, nor monkey-patching any APIs.
