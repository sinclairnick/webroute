...
Because `webroute` prohibits modifying a `Request` object, this is impossible. Instead we can just return a state object. This will be accessible with full-type safety in successive middleware and handlers.
...
<Accordions>
<Accordion title="Appending to state, instead of overwriting">
Bear in mind returning an object will _overwrite_ any existing state. So if we wish to keep the existing state, we can decide how we want to merge the old with new state. Typically that will look something like:
