...
<Callout>
Web frameworks typically encourage exporting a single app instance from one filesystem route. However, this circumvents all the benefits of filesystem routing.
</Callout>
...
<Callout>
With that said, we have provided some starting points in [Deployment](./deployment).
</Callout>
